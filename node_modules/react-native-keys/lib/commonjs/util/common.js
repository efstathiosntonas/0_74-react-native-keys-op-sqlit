...
var _process$env, _process$env$SRCROOT;
...
const exampleDirName = process.cwd().includes(expoExampleDirName) || (_process$env = process.env) !== null && _process$env !== void 0 && (_process$env$SRCROOT = _process$env.SRCROOT) !== null && _process$env$SRCROOT !== void 0 && _process$env$SRCROOT.includes(expoExampleDirName) ? expoExampleDirName : 'example';
const PROJECT_ROOT_DIR_PATH = path.join(__dirname, isExample ? `../../${exampleDirName}/` : '../../../../');
...
const IOS_DIR_PATH = path.join(PROJECT_ROOT_DIR_PATH, isExample ? KEYS_IOS_EXAMPLE_PATH : KEYS_IOS_PATH);
const CPP_DIRECTORY_PATH = path.join(PROJECT_ROOT_DIR_PATH, isExample ? '../' : RN_KEYS_PATH, 'cpp');
const ANDROID_DIR_PATH = path.join(PROJECT_ROOT_DIR_PATH, isExample ? KEYS_ANDROID_EXAMPLE_PATH : KEYS_ANDROID_PATH, 'cpp');
const SRC_PATH = path.join(PROJECT_ROOT_DIR_PATH, isExample ? KEYS_SRC_EXAMPLE_PATH : KEYS_SRC_PATH);
const ANDROID_KEYS_DIR_PATH = path.join(PACKAGE_ROOT_DIR_PATH, 'android', 'src', 'main', 'java', 'com', 'reactnativekeysjsi');
...
module.exports.genTSType = allKeys => {
  let result = '// this file is auto generate, please do not modify\nexport type KeyTurboType = {';
  Object.keys((allKeys === null || allKeys === void 0 ? void 0 : allKeys.public) ?? {}).forEach(key => {
    result += `\n  ${key}: string;`;
  });
  if (!(allKeys !== null && allKeys !== void 0 && allKeys.public)) {
    result += '\n [key: string]: string;\n};\n\n';
  } else {
    result += '\n};\n\n';
  }
  result += 'export type KeyTurboSecuredType = {';
  Object.keys((allKeys === null || allKeys === void 0 ? void 0 : allKeys.secure) ?? {}).forEach(key => {
    result += `\n  ${key}: string;`;
  });
  if (!(allKeys !== null && allKeys !== void 0 && allKeys.secure)) {
    result += '\n [key: string]: string;\n};\n\n';
  } else {
    result += '\n};\n\n';
  }
...
module.exports.getIosEnvironmentFile = () => {
  try {
    ...
    } else if (process.env.DEBUG_KEYSFILE && process.env.CONFIGURATION === 'Debug') {
      const debugFile = process.env.DEBUG_KEYSFILE;
      return debugFile;
    } else if (process.env.RELEASE_KEYSFILE && process.env.CONFIGURATION === 'Release') {
